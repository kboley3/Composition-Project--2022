...
def get_rho(Planet,grids,Core_wt_per,layers, dry,magma):
    """
        ...
        if num_core_layers > 0:
            ...
            if magma:
                for i in range(num_mantle_layers):
                    if i <= num_mantle_layers:
            
                        rho_layers[i+num_core_layers] = getMagmaRho(Pressure_layers[i+num_core_layers],Temperature_layers[i+num_core_layers],Core_wt_per, rho_layers[i+num_core_layers-1],dry)
...
def getMagmaRho(Pressure,Temperature,Core_wt_per, Lastrho, dry):
    """
   This module calculates the density of the mantle for a magma ocean planet given a list of pressures and temperatures and the light element
   composition of the core. See documentation for description of how light elements are treated.
    ...
    molar_weight_magma = (mol_frac_Fe*mFe) + (mol_frac_Si * mSi) + (mol_frac_O*mO) + (mol_frac_S*mS)
    
    #can be 'Baj', 'Sol3000', True, or False
    vinet='Sol3000'
    
    
    if dry==True and vinet==True:
        
        
        class MgSiO3(burnman.Mineral):
            
            #Fei et al (2021) 
            def __init__(self):
                self.params = {
                    'name': 'bridgmanite',
                    'equation_of_state': 'vinetD',
                    'rho_0': 4.176*1000 ,
                    'K_0': 265.5e9,
                    'Kprime_0': 4.16,
                    'gamma0':1.57,
                    'Theta0':1000,
                    'q':0.5,
                    'molar_mass': molar_weight_magma/1000,
                }
                burnman.Mineral.__init__(self)
    elif dry==True and vinet==False: 
        ...
        class MgSiO3(burnman.Mineral):
            ...
            def __init__(self):
                self.params = {
                    'name': 'bridgmanite',
                    'equation_of_state': 'bm4D',
                    'rho_0': 2.61*1000,
                    'K_0': 27e9,
                    'Kprime_0': 3.7,
                    'Kprime_prime_0': 0.06e-9,
                    'molar_mass': molar_weight_magma/1000,
                }
                burnman.Mineral.__init__(self)
    elif dry==True and vinet=='Sol3000': 
        
        class MgSiO3(burnman.Mineral):
            
            #Solomatova 2020 300K
            def __init__(self):
                self.params = {
                    'name': 'bridgmanite',
                    'equation_of_state': 'bm3D',
        ...
        class MgSiO3(burnman.Mineral):
            ...
            def __init__(self):
                ...
                burnman.Mineral.__init__(self)
    elif dry==2.2 and vinet=='Sol3000': 
        ...
        class MgSiO3(burnman.Mineral):
            ...
            def __init__(self):
                self.params = {
                    ...
                    'K_0': 18e9,
                    'Kprime_0': 5.0,
                    'Kprime_prime_0': -0.19e-9,
                    'molar_mass': molar_weight_magma/1000,
                }
                burnman.Mineral.__init__(self)
    elif dry==2.2 and vinet==False:  
        ...
        class MgSiO3(burnman.Mineral):
            ...
            def __init__(self):
                ...
                burnman.Mineral.__init__(self)
    elif dry==5.2 and vinet=='Sol3000': 
    ...
    #Lastrho=5*1000 #change
    if dry==True and Lastrho <= 4.176*1000: 
        #print(Lastrho/1000, 'Too low. Using last density')
        density = Lastrho
    elif dry!=True and Lastrho <= 2.88*1000:
        #print(Lastrho/1000, 'Too low. Using last density')
        density = Lastrho
    elif dry==3 and Lastrho <= 2.14*1000:
        #print(Lastrho/1000, 'Too low. Using last density')
        density = Lastrho
    else:
        density = rock.evaluate(['density'], 1.e9*(Pressure/10000.), Temperature)[0]
...
def get_temperature(Planet,grids,structural_parameters,layers,magma):
    """
    ...
    if magma:
        rho=Planet.get('density')
        gamma=1.57#1.57 # Fei et al paper
        q=0.5
        temperature=np.ones(num_mantle_layers+num_core_layers)*2000
        T0=300
        rho0=2.26*1000
        temperature[num_core_layers:]= T0*np.exp(gamma*(rho[num_core_layers:]/rho0))
        #temperature[num_core_layers:]= T0*np.exp(gamma*((rho0 *(1 + (-1 + q) *np.log(rho0)))/(-1 + q)**2 - ((rho0/rho[num_core_layers:])**q *rho[num_core_layers:]* (1 + (-1 + q)*np.log(rho[num_core_layers:])))/(-1 + q)**2))
        #temperature[num_core_layers:]= T0*np.exp(((gamma*rho0**q)/q)*(rho[num_core_layers:]**(-q)))
        return temperature
